...
class OSIGridAnalyzer:
    ...
    def analyze_grid_cells(self, segmentation_mask: np.ndarray, 
                          affected_mask: np.ndarray, grid: List[List[Tuple]],
                          nail_bbox: Tuple = None) -> Dict:
        """
        ...
        Args:
            ...
            nail_bbox (Tuple): Nail bounding box (x, y, w, h) for accurate area calculation
            
        Returns:
            Dict: Analysis results with area percentage and proximity info
        """
        total_affected_area = np.sum(affected_mask > 0) if affected_mask is not None else 0
        
        # Calculate nail area - prefer segmentation mask if available
        # The segmentation_mask represents the actual nail detected by the model
        seg_area_pixels = np.sum(segmentation_mask > 0) if segmentation_mask is not None else 0
        
        if seg_area_pixels > 0:
            # Use actual nail segmentation pixels (most accurate)
            total_nail_area = seg_area_pixels
            use_source = "nail segmentation mask"
        elif nail_bbox is not None:
            # Fall back to bounding box area
            x, y, w, h = nail_bbox
            total_nail_area = w * h
            use_source = f"bbox area ({w}x{h})"
        else:
            # Last resort: use full image area
            if affected_mask is not None:
                img_height, img_width = affected_mask.shape[0], affected_mask.shape[1]
            elif segmentation_mask is not None:
                img_height, img_width = segmentation_mask.shape[0], segmentation_mask.shape[1]
            else:
                img_height, img_width = 512, 512  # Default fallback
            total_nail_area = img_height * img_width
            use_source = f"full image ({img_width}x{img_height})"
        
        # Debug: print mask statistics
        if affected_mask is not None:
            affected_min = np.min(affected_mask)
            affected_max = np.max(affected_mask)
            affected_nonzero = np.sum(affected_mask > 0)
        else:
            affected_min = 0
            affected_max = 0
            affected_nonzero = 0
        
        # print(f"      [DEBUG] Using {use_source}, Affected: {affected_nonzero}, Total: {total_nail_area}, Percentage: {area_percent:.1f}%")
        ...
        if total_nail_area > 0:
            ...
            print(f"      [DEBUG] Percentage: {area_percent:.1f}%")
        else:
            # No image data - treat as healthy
            area_percent = 0
        ...
        if affected_mask is None:
            proximity_level = 1  # No affected area = distal
        else:
...
def get_osi_score(area_percent: float, proximity_level: int) -> Dict:
    ...
    if area_percent == 0:
        area_score = 0
    elif area_percent < 11:  # 1-10.99%
        area_score = 1
    elif area_percent < 26:  # 11-25.99%
        area_score = 2
    elif area_percent < 51:  # 26-50.99%
        area_score = 3
    elif area_percent < 76:  # 51-75.99%
        area_score = 4
    else:  # 76-100%
        area_score = 5
...
def process_nail_for_grading(cropped_nail_image: np.ndarray, 
    ...
    # Use nail_bbox from segmentation if provided (format: x1, y1, x2, y2)
    # This is the segmentation bounding box, which is more accurate for grading
    if nail_bbox_from_detection is not None:
        x1, y1, x2, y2 = nail_bbox_from_detection
        # Convert from (x1, y1, x2, y2) to (x, y, w, h) format for grid overlay
        nail_bbox = (x1, y1, x2 - x1, y2 - y1)
        # print(f"  [DEBUG] Using nail bbox from segmentation: original={nail_bbox_from_detection}, converted={nail_bbox}")
    else:
        # Try to detect nail contour from segmentation mask
        nail_contour, nail_bbox = analyzer.detect_nail_contour(seg_mask_uint8)
        
        # If contour detection fails, try using affected area bbox as fallback
        if nail_bbox is None and affected_mask is not None:
            print("  [WARNING] Nail contour detection failed, attempting to use affected area bbox as fallback")
            # Get affected area contour
            affected_uint8 = (affected_mask * 255).astype(np.uint8) if affected_mask.max() <= 1 else affected_mask.astype(np.uint8)
            affected_contour, affected_bbox_tuple = analyzer.detect_nail_contour(affected_uint8)
            if affected_bbox_tuple is not None:
                nail_bbox = affected_bbox_tuple
                print(f"  [DEBUG] Using affected area bbox as fallback: {nail_bbox}")
            else:
                # Last resort: use full image as bounding box
                print("  [WARNING] Could not detect affected area bbox either, using full image bounds")
                h, w = cropped_nail_image.shape[:2]
                nail_bbox = (0, 0, w, h)
        elif nail_bbox is None:
            # No nail contour and no affected area - use full image
            print("  [WARNING] No nail or affected area detected, using full image bounds")
            h, w = cropped_nail_image.shape[:2]
            nail_bbox = (0, 0, w, h)
    ...
    # Analyze grid cells - pass nail_bbox for accurate area calculation
    grid_analysis = analyzer.analyze_grid_cells(seg_mask_uint8, aff_mask_uint8, grid, nail_bbox=nail_bbox)
