...
class DatabaseManagerV2:
    ...
    def _get_patient_folder_name(self, patient_name: str) -> str:
        """Sanitize patient name for use as folder name."""
        # Replace invalid characters with underscores
        invalid_chars = r'[<>:"/\\|?*]'
        import re
        return re.sub(invalid_chars, '_', patient_name).strip()
    ...
    def update_patient(self, patient_id: int, name: str = None, status: str = None):
        """Update patient name and/or status, renaming folder if name changes."""
        import shutil
        ...
        with self._connect() as con:
            ...
            old_name = current[0] if current else None
        ...
        if name and old_name and name != old_name:
            old_folder = os.path.join(self.data_dir, self._get_patient_folder_name(old_name))
            new_folder = os.path.join(self.data_dir, self._get_patient_folder_name(name))
    ...
    def delete_patient(self, patient_id: int):
        ...
        with self._connect() as con:
            ...
            # Get patient name for folder deletion
            cur.execute("SELECT name FROM patients WHERE id = ?", (patient_id,))
        ...
        # Delete patient folder if it exists
        if patient_name:
            folder_name = self._get_patient_folder_name(patient_name)
            patient_dir = os.path.join(self.data_dir, folder_name)
            ...
            for side in ["left", "right"]:
                ...
                for nail_idx, nail in enumerate(cropped_nails, 1):
                    ...
                    nail_images = self._save_nail_images(
                        patient_name, side, nail_number, nail
                    )
                ...
                if foot_image is not None:
                    foot_img_path = self._save_image(
                        patient_name, f"{side}_foot_full.jpg", foot_image
                    )
                ...
                if detection_viz is not None:
                    detection_path = self._save_image(
                        patient_name, f"{side}_foot_detection.jpg", detection_viz
                    )
    ...
    def delete_all_scans(self):
        """Delete all scans and their associated images and patient folders."""
        import shutil
    ...
    def _save_image(self, patient_name: str, filename: str, img_bgr: np.ndarray) -> str:
    ...
    def _save_nail_images(
        self, patient_name: str, side: str, nail_number: int, nail_data: Dict
    ) -> Dict[str, str]:
        """
        ...
        if original_img is not None:
            ...
            nail_images["original"] = self._save_image(patient_name, filename, original_img)
        ...
        if grid_viz is not None:
            ...
            nail_images["grid"] = self._save_image(patient_name, filename, grid_viz)
        ...
        if nail_seg is not None:
            ...
            nail_images["segmentation"] = self._save_image(patient_name, filename, nail_seg)
