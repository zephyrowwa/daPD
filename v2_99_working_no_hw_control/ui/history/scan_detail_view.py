...
class ScanDetailView(QWidget):
    ...
    def _create_nail_card(self, nail, images_dict, db):
        ...
        # Images: Original on left (BIGGER), Grid on right
        images_main_layout = QHBoxLayout()
        ...
        if isinstance(nail_img_dict, dict):
            # LEFT COLUMN: Original image (larger, about 220px)
            ...
            if original_path:
                ...
                if img is not None:
                    img_label = QLabel()
                    img_label.setAlignment(Qt.AlignCenter)
                    pixmap = self._cv_to_qpixmap(img, max_width=220)
                    img_label.setPixmap(pixmap)
                    
                    frame = QFrame()
                    frame.setStyleSheet("border: 1px solid #d1d5db; border-radius: 4px; background-color: white;")
                    frame_layout = QVBoxLayout(frame)
                    ...
                    title.setStyleSheet("font-size: 8px; color: #6b7280; padding: 2px; font-weight: 600;")
                    
                    left_col = QWidget()
                    left_col_layout = QVBoxLayout(left_col)
                    left_col_layout.setContentsMargins(0, 0, 0, 0)
                    left_col_layout.setSpacing(2)
                    left_col_layout.addWidget(frame)
                    left_col_layout.addWidget(title)
                    
                    images_main_layout.addWidget(left_col, stretch=0)
            ...
            if grid_path:
                ...
                if img is not None:
                    ...
                    right_col_layout.addWidget(frame)
                    right_col_layout.addWidget(title)
                    
                    images_main_layout.addWidget(right_col, stretch=0)
        
        images_main_layout.addStretch()
        layout.addLayout(images_main_layout)
